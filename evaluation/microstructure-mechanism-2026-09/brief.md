...
- Contract: `FINANCEMETA-MICROSTRUCTURE-MECHANISM-2026-v5` (supersedes v1 to v4; amendments A1-A28 and implementation defects D1-D12 logged in `experiment_contract.json`)
- Freeze identity: PR #57 head + tag `microstructure-freeze-v5` + CI artifact `microstructure-mechanism-contract-<sha>`
...
The machine-readable log is `experiment_contract.json` `freeze.amendments` (A1-A28), each entry carrying
...
`freeze.implementation_defects_corrected` (D1-D12). Five of them would have invalidated the comparison
