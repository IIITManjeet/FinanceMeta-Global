...
min_allocation_lots = 1 is a participation floor, not a guarantee. An order
whose share is under a whole lot gets nothing in pass 1 and only competes in
passes 2 and 3. At one lot that is just integer allocation. It is not a rule
that hands every resting order a lot; that would be a different mechanism.

Both functions are pure in (resting, demand): no simulator state, no clock, no
RNG. That is what makes any outcome difference attributable to the rule.
"""
...
def _allocate_pro_rata(resting: list[Resting], demand: int) -> dict[int, int]:
...
def allocate(mechanism: str, resting: list[Resting], demand: int) -> dict[int, int]:
    ...
    if mechanism == PRO_RATA:
        return _allocate_pro_rata(resting, demand)
