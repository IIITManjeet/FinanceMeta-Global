...
def test_identity_control_holds_inside_full_runs(probe) -> None:
    fifo = run_once(probe, FIFO, seed=3, latency_ms=5)
    prorata = run_once(probe, PRO_RATA, seed=3, latency_ms=5)
    assert fifo.stream_sha256 == prorata.stream_sha256


def test_different_seeds_produce_different_streams(probe) -> None:
    digests = {stream_digest(generate_stream(probe, s, 2000)) for s in range(5)}
    assert len(digests) == 5


def test_deterministic_replay_is_byte_identical(probe) -> None:
    first = run_once(probe, FIFO, seed=7, latency_ms=5)
    second = run_once(probe, FIFO, seed=7, latency_ms=5)
...
def test_deterministic_replay_holds_for_pro_rata(probe) -> None:
    first = run_once(probe, PRO_RATA, seed=11, latency_ms=25)
    second = run_once(probe, PRO_RATA, seed=11, latency_ms=25)
